...
The Segment, Bimix, and Trimix samples were designed as complementary time-artifact stress tests. Segment samples model sustained acquisition shifts by appending events from one or more samples onto an existing sample. Bimix and Trimix samples model subtler mid-acquisition changes in event origin or fluorescence distribution by randomly combining events from two or three differently stained samples in 5000- or 2000-event mixing bins. While major blockages often produce segmental deviations, short self-resolving micro-blockages can generate smaller transient changes; the Bimix and Trimix simulations were designed to test detection of these less obvious distributional perturbations under controlled ground-truth conditions.
...
Most substantially, FlowMOP differs from the existing methods by choosing to measure how each time bin’s positive events differ, rather than re-calculating per time bin the ‘positive’ peak like in PeacoQC. Conversely, it also does not eschew positive peak detection completely (unless in Geomean mode) unlike FlowCut. Finally, FlowMOP employs smoothing at multiple resolutions to detect short spikes, sustained signal shifts, gradual drift, transient microblockages, and mixed-source acquisition irregularities, with parameter voting used to reduce excessive removal in higher-dimensional sample sets.
...
Clone-based runtime and memory benchmarking showed that FlowMOP achieved faster processing at lower peak memory use across larger event counts (Table S3). At 1,000,000 events, FlowMOP completed in a median 10.63 s, compared with 24.29 s for PeacoQC and 30.20 s for FlowCut, while using 658 MB peak memory compared with 1,433 MB and 1,115 MB respectively. At 2,000,000 events, FlowMOP completed in 16.96 s, compared with 41.26 s and 61.55 s, while using 840 MB peak memory compared with 2,615 MB and 1,850 MB. The best-performing runtime and RAM value for each event count is bolded in Table S3.
...
All algorithms were compared using recommended or default settings, including fixed FlowMOP parameters, to reflect typical unsupervised use. We did not perform extensive parameter tuning for FlowCut or PeacoQC because the original method descriptions do not provide dataset-specific guidance for how such tuning should be performed. Instead, we evaluate FlowMOP's sensitivity to the smoothing parameter in the MAD smoothing analysis, while treating full cross-method parameter optimization as outside the scope of this validation.
...
**Table S3: Clone-based time-gating runtime and memory benchmark**

Median runtime and peak RAM across three measured repeats are shown. The best-performing runtime and RAM value for each event count is shown in bold.

| Events | FlowMOP time (s) | PeacoQC time (s) | FlowCut time (s) | FlowMOP RAM (MB) | PeacoQC RAM (MB) | FlowCut RAM (MB) |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 10,000 | 2.68 | 4.10 | **1.73** | **238** | 447 | 262 |
| 100,000 | **4.40** | 13.46 | 4.65 | 352 | 618 | **326** |
| 300,000 | **8.71** | 15.21 | 10.67 | **462** | 952 | 506 |
| 1,000,000 | **10.63** | 24.29 | 30.20 | **658** | 1,433 | 1,115 |
| 2,000,000 | **16.96** | 41.26 | 61.55 | **840** | 2,615 | 1,850 |
