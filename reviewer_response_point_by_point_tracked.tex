...
revised {\color{revisionred}\sout{Introduction
discusses}}{\color{revisionblue}Abstract describes alternative
preprocessing algorithms such as} FlowCut and
{\color{revisionred}\sout{PeacoQC}}{\color{revisionblue}PeacoQC, while
the Introduction discusses them} as time-dependent quality-control
approaches and
{\color{revisionred}\sout{GateNet}}{\color{revisionblue}distinguishes
GateNet, UNITO,} and
{\color{revisionred}\sout{UNITO}}{\color{revisionblue}FATE} as broader
{\color{revisionred}\sout{automated
population-gating}}{\color{revisionblue}population-identification or
...
controls developed to provide objective ground truth for
{\color{revisionred}\sout{flow-cytometry}}{\color{revisionblue}flow
cytometry} preprocessing tasks.
...
\item
  ...
  including {\color{revisionblue}deep-learning approaches {[}7,8{]},}
  the model-based flowClust method
  {\color{revisionred}\sout{{[}13{]},}}{\color{revisionblue}{[}9{]},}
  the neural-network method GateNet
  {\color{revisionred}\sout{{[}14{]},}}{\color{revisionblue}{[}10{]},}
  and the bivariate-segmentation framework UNITO
  {\color{revisionred}\sout{{[}15{]};}}{\color{revisionblue}{[}11{]},
  while FATE uses representation learning to produce generalized flow
  cytometry embeddings {[}12{]};} automated gating approaches are
  reviewed elsewhere
  {\color{revisionred}\sout{{[}16{]}.}}{\color{revisionblue}{[}13{]}.
  Conversely,} FlowMOP is a Python-based, training-free preprocessing
  workflow that combines automated time-gating, debris removal, and
  doublet exclusion in a single headless tool. FlowCut and PeacoQC
  provide the most direct comparison for its time-gating component.''
  \end{quote}
\item
...
\item
  ...
  decisions
  {\color{revisionred}\sout{{[}5,9{]}.}}{\color{revisionblue}{[}5,6{]}.}
  ...
  same independent channel-wise reduction without changing their
  decision rules. Porting either implementation to another language or
  scheduling framework would therefore not remove these structural
  dependencies. This structural distinction, together with FlowMOP's
  earlier data reduction, fewer full-data passes, and smaller
  intermediate state, is consistent with its lower runtime and memory
  use in our benchmarks, although the benchmark does not independently
  establish causation.''
  \end{quote}
\item
...
\textbf{Location:} Methods, {\color{revisionblue}``Computational
scalability benchmark''; Results,} ``Algorithmic design'' and
``Computational scalability'';{\color{revisionred}\sout{Results,
``Computational scalability'';}} Table 1; Discussion, ``Synthetic Sample
Time Gating.''
...
samples,}}{\color{revisionblue}added PBMC samples from eight donors and
three human liver-tumour samples. These analyses quantify PBMC
population recovery and biological composition} and
{\color{revisionred}\sout{the experimental relevance of the synthetic
benchmark.}}{\color{revisionblue}tumour-population recovery.}
...
{\color{revisionred}\sout{final PBMC cohort description, population
definitions, results, statistical
comparisons,}}{\color{revisionblue}effects of debris removal. To expose
those effects directly, we did not include CD45+ as the parent of the
lineage populations. B cells were defined as Live CD3−CD19+ events, T
cells as Live CD3+CD19− events,} and {\color{revisionred}\sout{Figure 5
references will be inserted here when}}{\color{revisionblue}NKT cells as
Live CD19−CD3+CD56+ events. We report counts and frequencies normalized
to their corresponding matched Raw values (Raw = 1); before
normalization, B-, T-, and NKT-cell frequencies were calculated relative
to Live cells. All analyses used paired PBMC samples from} the
{\color{revisionred}\sout{analysis is
complete.}}{\color{revisionblue}same eight donors. Direct workflow
comparisons used two-sided paired \emph{t}-tests, while Raw comparisons
used one-sample \emph{t}-tests against 1 on the Raw-normalized values,
equivalent to paired comparisons with the matched Raw values; tests were
Holm-adjusted separately within endpoint and outcome metric.}

{\color{revisionred}\sout{We compared}}{\color{revisionblue}For time
gating, no evaluated B-, T-, or NKT-cell frequency differed
significantly among} the {\color{revisionred}\sout{Raw data with
sequential manual preprocessing and the
complete}}{\color{revisionblue}methods (all adjusted p values were 0.420
or greater). Relative to Expert Manual,} FlowMOP
{\color{revisionred}\sout{output, in which independently calculated
time, debris, and doublet exclusions are combined as a union of excluded
events. We then quantified four prespecified downstream endpoints: the
T:B-cell ratio, Live CD45+ cell count, B-cell
frequency,}}{\color{revisionblue}retained 11.1--12.7 percentage points
more B, T,} and {\color{revisionred}\sout{T-cell frequency.
T}}{\color{revisionblue}NKT} cells {\color{revisionblue}(all adjusted p
values} were {\color{revisionred}\sout{defined as CD3+CD19− and B cells
as CD3−CD19+. Each endpoint was normalized within sample to its matched
Raw value,}}{\color{revisionblue}0.003 or less), FlowCut retained
12.3--13.8 points more (all adjusted p values were 0.006 or less),} and
...
contrasts the sequential manual strategy}}{\color{revisionblue}contains
representative debris and doublet preprocessing projections,} with
{\color{revisionred}\sout{FlowMOP\textquotesingle s parallel exclusion
logic.}}{\color{revisionblue}gate outlines shown only where the
corresponding preprocessing gate was applied.}

{\color{revisionred}\sout{These biological analyses complement, but do
not convert,}}{\color{revisionblue}In the tumour analysis,} the
{\color{revisionred}\sout{earlier forced expert rankings into an
objective benchmark. The expert-ranking exercise remains an exploratory
comparison of}}{\color{revisionblue}B- and T-cell endpoints are
population recovery} relative
{\color{revisionred}\sout{preferences}}{\color{revisionblue}to Raw, not
frequency or composition within the cleaned output, because their source
counts use the original total-event denominator. No statistically
detectable differences were observed between Manual} and
{\color{revisionred}\sout{is addressed
separately}}{\color{revisionblue}FlowMOP for the evaluated recovery
endpoints} in {\color{revisionred}\sout{Combined Comment
6.}}{\color{revisionblue}these three samples (Figure 7). Given n = 3,
these nonsignificant results do not establish equivalence.}
...
\tightlist
\item
  We added a {\color{revisionblue}biological validation using} PBMC
  {\color{revisionred}\sout{biological-validation section based on
  prespecified downstream population endpoints; the final
  analysis}}{\color{revisionblue}samples from eight donors, with
  ...
  preprocessing projections; gate outlines appear only where the
  corresponding preprocessing gate was applied.}
\end{itemize}
...
\item
  ...
  of{\color{revisionred}\sout{the T:B-cell ratio,}} Live CD45+
  {\color{revisionred}\sout{cell count, B-cell
  frequency,}}{\color{revisionblue}cell-count recovery and B-} and
  T-cell
  {\color{revisionred}\sout{frequency.}}{\color{revisionblue}population
  recovery.} Figure {\color{revisionred}\sout{6}}{\color{revisionblue}7}
  displays the matched gating plots and {\color{revisionred}\sout{the
  Live CD45+ cell-count, B-cell-frequency, and
  T-cell-frequency}}{\color{revisionblue}downstream} comparisons.
  {\color{revisionblue}No statistically detectable differences were
  observed between Manual and FlowMOP for these recovery endpoints, but
  n = 3 does not support an equivalence claim.}
\item
...
Among the smoothed settings,
{\color{revisionred}\sout{\texttt{0.01,0.05}}}{\color{revisionblue}0.01,0.05}
provided the strongest balance and was selected as the default. We reran
FlowMOP across all Figure 2 inputs and regenerated Figure 2 using these
outputs.
...
\item
  ...
  default
  {\color{revisionred}\sout{\texttt{0.01,0.05}),}}{\color{revisionblue}0.01,0.05),}
  are applied to the returned time-bin series before median absolute
  deviation (MAD) filtering.''
  \end{quote}
...
\item
  ...
  and identifies
  {\color{revisionred}\sout{\texttt{0.01,0.05}}}{\color{revisionblue}0.01,0.05}
  as the strongest-performing smoothed setting on the balanced
  comparison.
\item
  We reran FlowMOP using the selected
  {\color{revisionred}\sout{\texttt{0.01,0.05}}}{\color{revisionblue}0.01,0.05}
  setting and regenerated the Figure 2 violin distributions, p-values,
  and significance brackets from these outputs.
\item
...
2; Figure S4;{\color{revisionred}\sout{Supplementary}} Table S4;
...
produced distinct} results {\color{revisionblue}across the three
cleaning modules. For time gating, FlowMOP had the best mean rank among
the automated methods and was preferred to FlowCut (BF = 5.39, P =
84.3\%) and PeacoQC (BF = 12.10, P = 92.4\%). Human gates generally
ranked higher for debris and doublet removal, although FlowMOP was
competitive in several tissue-specific comparisons: it ranked first for
debris removal in mouse blood, third for debris removal in human liver
and mouse skin, and second for doublet removal in human liver (Figs.
S5-S7). We now report these principal findings and statistics in the
main Results; the complete dataset-level rankings and mean ranks} are
{\color{revisionred}\sout{presented as exploratory evidence of expert
preference rather than objective ground-truth evidence of gating quality
or algorithmic superiority.}}{\color{revisionblue}shown in Figures
S5-S7, with statistical comparisons reported in the Supplementary
Results.}
...
\item
  ...
  gating
  {\color{revisionred}\sout{adequacy.''}}{\color{revisionblue}adequacy
  {[}16{]}.''}
  \end{quote}
\item
...
evaluation''; Discussion; Conclusion;} Supplementary
{\color{revisionred}\sout{Information,}}{\color{revisionblue}data,}
...
\item
  ...
  ``Modern
  {\color{revisionred}\sout{flow-cytometry}}{\color{revisionblue}flow
  cytometry} studies can contain increasing numbers of files, events,
  and measured parameters
  {\color{revisionred}\sout{{[}2{]}.}}{\color{revisionblue}{[}1{]}.} The
  large scale of these data makes repeated manual preprocessing
  time-consuming and often impractical, potentially amplifying
  variability between operators. Reproducible automated preprocessing is
  therefore valuable for large studies.''
  \end{quote}
\end{itemize}
...
\item
  ...
  dataset-specific} acquisition {\color{revisionred}\sout{voltage/gain
  information remains a PLACEHOLDER.}}{\color{revisionblue}settings in
  the Methods:}

  {\color{revisionred}\sout{- We did not add a voltage/gain-series
  experiment because}}{\color{revisionblue}\textgreater{} ``The
  ...
  settings. Consequently,} the
  {\color{revisionred}\sout{tested}}{\color{revisionblue}antigen--fluorochrome
  combinations and acquisition voltage/gain} settings
  {\color{revisionred}\sout{would be specific to the instrument,
  detector, panel, sample type,}}{\color{revisionblue}differ among
  datasets} and {\color{revisionred}\sout{experimental conditions rather
  than broadly generalisable. Instead, we
  added}}{\color{revisionblue}were not variables under evaluation.''}
\end{itemize}

{\color{revisionblue}- We retained} the following combined
...
\textbf{Location:} {\color{revisionblue}Methods, ``Synthetic time
source-sample preparation'';} Discussion, ``Other
{\color{revisionred}\sout{remarks''; acquisition voltage/gain
...
association with signal-intensity variation
{\color{revisionred}\sout{{[}10{]}.}}{\color{revisionblue}{[}4{]}.}
PeacoQC describes temporary acquisition shifts caused by slow sample
uptake or a clog and notes that such problems can be difficult to detect
manually {[}5{]}, while FlowCut describes manual identification and
removal of transient acquisition problems as time-consuming and
subjective
{\color{revisionred}\sout{{[}9{]}.}}{\color{revisionblue}{[}6{]}.}
...
\item
  ...
  detect manually {[}5{]}, and FlowCut describes manual identification
  and removal of transient acquisition problems as time-consuming and
  subjective
  {\color{revisionred}\sout{{[}9{]}.}}{\color{revisionblue}{[}6{]}.} We
  use ``microblockage'' operationally for a short, self-resolving
  instance of this broader phenomenon that produces a localized
  fluorescence shift, without asserting that a physical obstruction was
  directly observed.
  \end{quote}
...
{\color{revisionred}\sout{S4.}}{\color{revisionblue}S4; Table S1A.} The
...
{\color{revisionred}\sout{Preservation of}}{\color{revisionblue}Figure 3
compares FlowMOP with four expert debris gates using the source-labelled
controls. Figure 6 complements this comparison with total debris-gate
removal and} downstream {\color{revisionblue}biological preservation in
paired} PBMC {\color{revisionred}\sout{population structure
and}}{\color{revisionblue}samples from eight donors. Across all events,
FlowMOP excluded 3.3--56.7\% of} the
{\color{revisionred}\sout{difficult-sample tumour
...
between} the {\color{revisionred}\sout{debris-method
description.}}{\color{revisionblue}methods. No population frequency
differed from Expert Manual except T-cell frequency, which was lower
after FlowMOP cleaning (Raw-normalized mean 1.013 versus 1.069;
Holm-adjusted p = 0.037). Figure 6 reports the downstream plots, and
Supplementary Figure S8 shows the representative debris and doublet
preprocessing projections. The source-labelled synthetic debris
benchmark separately provides the objective assessment of targeted
debris removal.}
...
represent{\color{revisionred}\sout{only}} heterologous labelled
{\color{revisionred}\sout{doublets.}}{\color{revisionblue}doublets, with
rare dye-transfer events as a possible exception.} CTV-CTV and CFSE-CFSE
...
doublet cleaning in paired PBMC samples from eight donors. FlowMOP
...
Figure S8 shows the representative doublet preprocessing projections.}
...
\item
  ...
  \begin{quote}
  {\color{revisionred}\sout{``Events}}{\color{revisionblue}``Because the
  CTV- and CFSE-labelled populations were generated separately, events}
  positive for both {\color{revisionred}\sout{CFSE and CTV provide an
  observable class of}}{\color{revisionblue}dyes were interpreted as}
  heterologous {\color{revisionred}\sout{labelled doublets, subject
  to}}{\color{revisionblue}doublets;} rare dye-transfer
  {\color{revisionred}\sout{events;
  same-label}}{\color{revisionblue}events are a possible exception.
...
\item
  ...
  variables under {\color{revisionred}\sout{evaluation; they are not
  material to the preprocessing questions addressed
  here.''}}{\color{revisionblue}evaluation.''}
  \end{quote}
\end{itemize}

\textbf{Location:} Methods, {\color{revisionred}\sout{``Preparation of
Human PBMC Samples for Synthetic Time Benchmarking
Samples.''}}{\color{revisionblue}``Synthetic time source-sample
preparation.''}
...
{\color{revisionred}\sout{The number and percentage of events removed
by}}{\color{revisionblue}We calculated}
this{\color{revisionred}\sout{specific precleaning}} step
{\color{revisionred}\sout{are}}{\color{revisionblue}directly from} the
{\color{revisionred}\sout{relevant quantities. A dataset-level summary
of those}}{\color{revisionblue}passed\_lod channel, treating} values
{\color{revisionred}\sout{is}}{\color{revisionblue}below 0.5 as removed.
The rule did} not
{\color{revisionred}\sout{yet}}{\color{revisionblue}activate in PBMC
samples from any of the eight donors. In the three tumour samples, it
removed 3,472--9,206 events, corresponding to 2.39--3.05\% of each
input. The} complete {\color{revisionblue}sample-level counts} and
{\color{revisionred}\sout{remains to be added before submission;
percentage-based retention results from later gating
stages}}{\color{revisionblue}percentages} are
{\color{revisionred}\sout{not presented as a substitute for this
precleaning summary.}}{\color{revisionblue}reported in Supplementary
Table S3.}

The 1\% cutoff is {\color{revisionred}\sout{a pragmatic safeguard rather
than a uniquely ground-truth-derived boundary. It is low enough to
identify files with a non-trivial accumulation of events at the
acquisition maximum while avoiding removal triggered by isolated
maximum-valued events. The revised Results state
this}}{\color{revisionblue}an arbitrary} operational
{\color{revisionred}\sout{rationale.}}{\color{revisionblue}safeguard,
not an empirically optimized biological threshold. Its activation
depends on acquisition scaling and voltage/gain settings, and the
observed activation rates are dataset-specific and should not be
generalized across instruments.}
...
\item
  ...
  \end{quote}
\item
  We added the following operational
  {\color{revisionred}\sout{rationale:}}{\color{revisionblue}rationale
  and dataset-specific qualification:}

  \begin{quote}
  ``The 1\% cutoff is {\color{revisionred}\sout{a pragmatic safeguard
  intended to identify non-trivial accumulation
  at}}{\color{revisionblue}an arbitrary operational safeguard, not an
  empirically optimized biological threshold. In} the
  {\color{revisionblue}biological-validation inputs, the rule did not
  activate in any of the eight PBMC samples, whereas it removed
  3,472--9,206 events, or 2.39--3.05\%, in the three tumour samples
  (Table S3). Activation depends on} acquisition
  {\color{revisionred}\sout{maximum without responding to isolated
  maximum-valued events.''}}{\color{revisionblue}scaling and
  ...
  \end{quote}
\item
  {\color{revisionred}\sout{The dataset-level precleaning removal counts
  and percentages remain to be}}{\color{revisionblue}We} added
  {\color{revisionred}\sout{before
  submission.}}{\color{revisionblue}Supplementary Table S3 with the
  input events, events removed, percentage removed, and
  threshold-activation status for every biological-validation input.}
\end{itemize}

\textbf{Location:} Results,
{\color{revisionred}\sout{``Precleaning.''}}{\color{revisionblue}``Precleaning'';
