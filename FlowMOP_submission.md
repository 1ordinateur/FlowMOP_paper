...
Dual-resolution smoothing increased sensitivity, with a corresponding specificity trade-off (Table S5). Relative to no smoothing, the current dual-resolution default (`0.01,0.05`) increased mean sensitivity by 3.1%, while mean specificity decreased by 3.6%. Across the tested dual settings, `0.02,0.09` produced the highest sensitivity (0.9122), whereas the no-smoothing control produced the highest specificity (0.6400).
...
**Table S5: FlowMOP MAD-smoothing ablation on the 92 largecut synthetic time-gating files**

| Short, long smoothing factors | Runs | Sensitivity, mean ± SD | Specificity, mean ± SD |
| --- | ---: | ---: | ---: |
...
Sensitivity and specificity follow the definitions used for Figure 2. Bold indicates the highest mean in each metric. Relative enrichment is the percentage change in mean sensitivity from the no-smoothing control.
