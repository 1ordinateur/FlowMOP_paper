...
\hypersetup{
  ...
  pdftitle={FlowMOP: An Automated Flow Cytometry Time, Debris, and
...
\fancyhead[R]{\small\sffamily Revised manuscript}
...
\begin{center}
  {\LARGE\bfseries\sffamily FlowMOP: An Automated Flow Cytometry Time,
Debris, and Doublet Removal Tool\par}
  \vspace{7pt}
  {\large\sffamily\color{linkblue} Revised manuscript\par}
\end{center}
\vspace{8pt}
...
Tony Xu {[}1{]}, Nadia A. Roberts {[}1{]}, Felix Marsh-Wakefield
{[}2,3{]}, Rebecca A. Jaeger {[}1{]}, Sarah Croft {[}1{]}, Abhimanu
Pandey {[}1{]}, Dalton Leibold {[}4{]}, Angela L. Ferguson {[}2,3{]},
Lily Rodgers {[}2,3{]}, Umaimainthan Palendira {[}3{]}, Geoffrey W.
McCaughan {[}2,5,6{]}, Ben Quah {[}7{]}, Robin Vlieger* {[}8{]}, Anne
Brüstle* {[}1{]}

\textbf{Affiliations:}

1: Department of Immunology and Infectious Disease, John Curtin School
of Medical Research, the Australian National University, Canberra,
Australian Capital Territory, Australia 2: Liver Injury \& Cancer
Program, Cancer Innovations Centre, Centenary Institute, The University
of Sydney, Sydney, New South Wales, Australia 3: Human Immunology
Laboratory, School of Medical Sciences, Faculty of Medicine and Health,
The University of Sydney, Sydney NSW, Australia 4: Division of Ecology
and Evolution, Research School of Biology, the Australian National
University, Canberra, Australian Capital Territory, Australia 5:~A.W.
Morrow Gastroenterology and Liver Centre, Royal Prince Alfred Hospital,
Sydney NSW, Australia 6:~Sydney Medical School , Faculty of Medicine and
Health, The University of Sydney, Sydney NSW, Australia 7: Division of
Genome Sciences and Cancer, John Curtin School of Medical Research, the
Australian National University, Canberra, Australian Capital Territory,
Australia 8: School of Medicine and Psychology, Australian National
University, Canberra, Australian Capital Territory, Australia

\textbf{Contact information for all authors:}
...
TX is supported by the Australian Government Research Training Program
PhD Scholarship. There are no other relevant sources of funding.
...
memory-conscious summary calculations.
...
\subsubsection{Preparation protocols for synthetic
samples}\label{preparation-protocols-for-synthetic-samples}

\paragraph{Synthetic Time Sample Preparation and
Generation}\label{synthetic-time-sample-preparation-and-generation}

\paragraph{Preparation of Human PBMC Samples for Synthetic Time
Benchmarking
...
\paragraph{Generation of Synthetic Time
...
2000 events. Segmented samples model sustained changes, whereas Bimix
and Trimix provide controlled tests of subtler mid-acquisition changes.
...
\paragraph{Time-only acquisition-rate mechanism
...
\paragraph{MAD-smoothing ablation and default
...
synthetic time-gating files with all non-smoothing settings fixed. Ten
short/long smoothing-factor pairs were evaluated, including a
\texttt{0,0} interpolation control. The primary comparison in Figure 2
used the then-default pair (\texttt{0.1,0.9}); this separate ablation
informed the current default (\texttt{0.01,0.05}) but was not used to
rerun the primary comparison (Table S5).

\paragraph{Mice}\label{mice}
...
\paragraph{Synthetic Debris Sample Preparation and
...
\paragraph{Synthetic Doublet Sample Preparation and
...
\subsubsection{Non-synthetic samples}\label{non-synthetic-samples}
...
\subsubsection{Tumour-liver acquisition-instability and
...
\subsubsection{Statistical Analysis of Expert Preference
...
Bayes factor BF₁₀ = p/(1 -- p) under equal prior odds, interpreted using
Jeffreys' scale and reported as BF, P (Bayes factor, Posterior
...
\subsubsection{Coding Assistance}\label{coding-assistance}
...
\subsubsection{Algorithmic design}\label{algorithmic-design}
...
\paragraph{Precleaning}\label{precleaning}
...
\paragraph{Time Gating}\label{time-gating}
...
\paragraph{Debris Gating}\label{debris-gating}
...
instrument-dependent. This module is not intended to classify all
...
\paragraph{Doublet Gating}\label{doublet-gating}
...
\subsubsection{Algorithmic Validation}\label{algorithmic-validation}

\paragraph{Synthetic Sample
...
\paragraph{Synthetic Time Gating
...
The smoothing ablation did not identify a sharply defined optimum (Table
S5). The highest tested mean balanced score was 0.7629 for
\texttt{0.02,0.05}, compared with 0.7623 for the current
\texttt{0.01,0.05} default and 0.7603 for the interpolation control.
This broad low-smoothing plateau does not support smoothing alone as the
explanation for FlowMOP\textquotesingle s comparative performance.

Representative plots for the 2000 bin Bimix method, and the 2000, and
5000 bin Trimix methods can be found in Supp. 1.

\paragraph{Computational scalability}\label{computational-scalability}
...
\toprule\noalign{}
Events & FlowMOP time (s) & PeacoQC time (s) & FlowCut time (s) &
FlowMOP RAM (MB) & PeacoQC RAM (MB) & FlowCut RAM (MB) \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\paragraph{Synthetic Debris Gating
...
\paragraph{Synthetic Doublet Gating
...
\subsubsection{Expert Preference
...
\subsubsection{Tumour-liver acquisition instability and downstream
biological
...
\subsubsection{Synthetic Benchmarking}\label{synthetic-benchmarking}

\paragraph{Synthetic Sample Time
...
The Bimix synthetic samples seek to emulate transient microblockages
that self-resolve quickly. The size of the simulated microblockage is
determined by the `bin size', where the larger bin sizes emulate larger
blockages. As expected, algorithm sensitivity and specificity reduced in
the smaller bin-sized samples. This degradation was particularly marked
for PeacoQC's sensitivity. PeacoQC had higher sensitivity than both
other algorithms at the larger bin size but lower sensitivity than
FlowCut in the smaller-bin runs. FlowMOP had higher specificity than
FlowCut for the large-bin Bimix samples. In the Trimix results, FlowMOP
had higher specificity than both other algorithms at the large bin size
and higher specificity than PeacoQC in the smaller-bin samples.
...
event-level reference for comparing algorithmic performance.

\paragraph{Synthetic Sample Debris and Doublet
...
debris can be identified from FSC-A alone. Larger aggregates or
tissue-specific contaminants may resemble intact large cells,
granulocytes, hepatocytes, or other biologically plausible populations
in FSC/SSC space, and distinguishing these events generally requires
domain knowledge or sample-specific gating. Future techniques may look
to define debris on a per-event methodology based on that event's entire
fluorescence profile, rather than a traditional `gate' that sets
universal thresholds. The present liver analysis provides an initial
test in tumour tissue, but further tumour digests with severe necrosis,
large aggregates, and heterogeneous scatter profiles remain important
validation contexts because they may challenge a conservative
FSC-A-centered debris strategy.

\subsubsection{Biological Datasets: Expert Preference
...
\subsubsection{Other remarks}\label{other-remarks}
...
\toprule\noalign{}
Dataset Name & Synthetic Time-Gating Dataset & Synthetic Debris-Gating
Dataset & Synthetic Doublet-Gating Dataset & Human Liver \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\begin{longtable}[]{@{}lllll@{}}
\toprule\noalign{}
Mouse Dorsal Root Ganglion & Mouse Skin & Mouse Small Intestine & Mouse
Colon & Mouse Brain \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
\begin{longtable}[]{@{}lllll@{}}
\toprule\noalign{}
Mouse Central Nervous System & Mouse Spleen & Mouse Blood & Mouse Bone
Marrow & Human Cultured T cells \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\begin{longtable}[]{@{}lllll@{}}
\toprule\noalign{}
Rankings & Mouse DRG & Mouse Skin & Human Cultured T Cells & Mouse Bone
Marrow \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
\toprule\noalign{}
Sample & Tissue & Manual retained (\%) & FlowMOP retained (\%) &
Zombie-high in raw (\%) & Zombie-high among FlowMOP-removed (\%) &
Zombie-low removed (\%) & Zombie-high removed (\%) & Manual selectivity
ratio & FlowMOP selectivity ratio & FlowMOP/manual fold-change \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
\toprule\noalign{}
Validation & Samples or model & Biological endpoint & Comparator &
Primary effect measure & Result \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
Specificity, mean ± SD & Balanced score, mean ± SD \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\texttt{0,0} (interpolation control) & 92 & 0.8805 ± 0.1799 &
\textbf{0.6400 ± 0.2035} & 0.7603 ± 0.1300 \\
\texttt{0.01,0.02} & 92 & 0.8888 ± 0.1710 & 0.6286 ± 0.2087 & 0.7587 ±
0.1219 \\
\texttt{0.01,0.05} (current default) & 92 & 0.9077 ± 0.1605 & 0.6168 ±
0.2137 & 0.7623 ± 0.1166 \\
\texttt{0.02,0.05} & 92 & 0.9079 ± 0.1588 & 0.6179 ± 0.2152 &
\textbf{0.7629 ± 0.1173} \\
\texttt{0.02,0.09} & 92 & \textbf{0.9122 ± 0.1558} & 0.6058 ± 0.2156 &
0.7590 ± 0.1155 \\
\texttt{0.02,0.10} & 92 & 0.9099 ± 0.1560 & 0.6027 ± 0.2153 & 0.7563 ±
0.1135 \\
\texttt{0.05,0.20} & 92 & 0.9084 ± 0.1550 & 0.5892 ± 0.2229 & 0.7488 ±
0.1115 \\
\texttt{0.10,0.90} (primary-comparison setting) & 92 & 0.8947 ± 0.1599 &
0.5712 ± 0.2188 & 0.7329 ± 0.1096 \\
\texttt{0.20,0.90} & 92 & 0.8691 ± 0.1701 & 0.5548 ± 0.2156 & 0.7120 ±
0.1128 \\
\texttt{0.40,0.90} & 92 & 0.8380 ± 0.1758 & 0.5688 ± 0.2196 & 0.7034 ±
0.1148 \\
\end{longtable}
}
...
balanced score is their unweighted mean. Bold indicates the highest mean
in each metric. The \texttt{0,0} setting is the interpolation control.
Because no independent optimization/validation split was prespecified,
this is a parameter-sensitivity ablation rather than evidence of a
universal optimum.
