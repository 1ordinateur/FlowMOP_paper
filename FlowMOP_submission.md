...
The Python implementation specifically facilitates integration with contemporary machine-learning workflows, which predominantly utilize Python-based frameworks. FlowMOP also supports scalable execution through Dask-compatible, chunk-oriented computation, providing a practical route for distributed preprocessing of large cytometry datasets. Computational scalability is evaluated separately from gating accuracy, as implementation speed does not by itself establish improved biological or analytical performance.
...
At the algorithmic level, FlowMOP is well suited to parallel execution because its time-gating procedure reduces event-level measurements to independent per-bin, per-channel summaries before applying smoothing, MAD-based outlier detection, and channel voting. These operations have a regular array structure and can be evaluated independently across files, channels, bins, and smoothing scales before a final reduction step.

PeacoQC and FlowCut also contain parallelisable components, particularly across files and channels. However, their core decision logic involves more irregular intermediate state. PeacoQC depends on peak identification and tracking across bins, while FlowCut uses segment-level decisions and optional iterative re-evaluation. These steps introduce more synchronisation around peak or segment interpretation than FlowMOP’s fixed bin-summary and voting structure.

Therefore, FlowMOP should not be interpreted as more accurate because it is Dask-compatible. Rather, its algorithmic structure and Python implementation make it more directly compatible with vectorised and distributed execution. Accuracy and biological validity are evaluated separately through synthetic ground-truth analyses, expert comparisons, and downstream validation.
