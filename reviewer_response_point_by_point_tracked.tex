...
samples,}}{\color{revisionblue}added eight human PBMC samples and three
human tumour-liver samples. These analyses quantify PBMC population
recovery and biological composition} and {\color{revisionred}\sout{the
experimental relevance of the synthetic
...
PBMC data}}{\color{revisionblue}parent gate excluded many low-scatter
events before the lineage endpoints were quantified} and
{\color{revisionred}\sout{prespecified downstream population endpoints.
This analysis evaluates whether cleaning
preserves}}{\color{revisionblue}therefore obscured} the
{\color{revisionred}\sout{B-cell-to-T-cell
ratio}}{\color{revisionblue}effects of debris removal. To expose those
effects directly, we did not include CD45+ as the parent of the lineage
populations. B cells were defined as Live CD3−CD19+ events, T cells as
Live CD3+CD19− events,} and {\color{revisionred}\sout{relevant
constituent populations, providing a direct biological readout rather
than another subjective gate-ranking exercise.
PLACEHOLDER:}}{\color{revisionblue}NKT cells as Live CD19−CD3+CD56+
events. We report counts and frequencies normalized to their
corresponding matched Raw values (Raw = 1); before normalization, B-,
T-, and NKT-cell frequencies were calculated relative to Live cells. All
analyses used} the
{\color{revisionred}\sout{final}}{\color{revisionblue}same eight paired}
PBMC {\color{revisionred}\sout{cohort description, population
definitions, results, statistical comparisons, and Figure 5 references
will be inserted here when}}{\color{revisionblue}inputs. Direct workflow
comparisons used two-sided paired \emph{t}-tests, while Raw comparisons
used one-sample \emph{t}-tests against 1 on} the
...
calculated time, debris,}}{\color{revisionblue}Figure 5 numerical
results} and {\color{revisionred}\sout{doublet
...
endpoints:}}{\color{revisionblue}reported in Supplementary Table S5A.
For time gating, no evaluated B-, T-, or NKT-cell frequency differed
significantly among} the {\color{revisionred}\sout{T:B-cell ratio, Live
CD45+ cell count, B-cell frequency, and T-cell frequency. T
cells}}{\color{revisionblue}methods (all adjusted p values} were
{\color{revisionred}\sout{defined as
CD3+CD19−}}{\color{revisionblue}0.420 or greater). Relative to Expert
...
subcolumns per population, with all contrasts tabulated in Supplementary
Table S5B.} FlowMOP {\color{revisionred}\sout{was detected for the
T:B-cell ratio (p = 0.636), Live CD45+
...
{\color{revisionred}\sout{Raw, p = 0.906; Manual versus
FlowMOP,}}{\color{revisionblue}1.022; adjusted} p =
{\color{revisionred}\sout{0.636).}}{\color{revisionblue}0.049).} These
results {\color{revisionred}\sout{support concordant downstream
conclusions in these three complex samples. The analysis compares the
complete preprocessing workflows}}{\color{revisionblue}indicate broad
agreement between human-} and {\color{revisionred}\sout{their downstream
...
\tightlist
\item
  We added a {\color{revisionblue}biological validation using eight}
  PBMC {\color{revisionred}\sout{biological-validation section based on
  prespecified downstream population endpoints; the final
  analysis}}{\color{revisionblue}samples, with matched-Raw frequencies
  and counts for Live CD45+, B, T,} and {\color{revisionblue}NKT cells.}
  Figure 5 {\color{revisionred}\sout{content remain a
  PLACEHOLDER.}}{\color{revisionblue}reports time-cleaning
  representatives (A), frequencies (B), and counts (C), with all
  contrasts in Supplementary Table S5A. Figure 6 reports the combined
  Time + Debris + Doublet representative (A) and the debris-only,
  doublet-only, and combined frequency (B) and count (C) comparisons,
  with all contrasts in Supplementary Table S5B. Supplementary Figure S8
  contains the debris and doublet representative gates only.}
\end{itemize}
...
6;}}{\color{revisionblue}5--7;} Figure
{\color{revisionred}\sout{S8.}}{\color{revisionblue}S8; Supplementary
Tables S5A and S5B.}
...
eight paired} PBMC {\color{revisionred}\sout{population structure
and}}{\color{revisionblue}inputs. Across all events, FlowMOP excluded
...
Holm-adjusted p = 0.037). Figure 6 reports the downstream plots,
Supplementary Table S5B provides all contrasts, and Supplementary Figure
S8 shows the representative debris and doublet gates. The
source-labelled synthetic debris benchmark separately provides the
objective assessment of targeted debris removal.}
...
{\color{revisionred}\sout{3.}}{\color{revisionblue}S8; Supplementary
Table S5B.}
...
doublet cleaning in the eight paired PBMC inputs. FlowMOP produced
slightly lower mean retained counts across the Live CD45+ reference, B-,
T-, and NKT-cell endpoints, but no doublet count or frequency endpoint
differed significantly between methods (Figure 6B,C; Supplementary Table
S5B). When Time, Debris, and Doublet preprocessing were combined, no
count endpoint differed; B-cell frequency was the only frequency that
differed between FlowMOP and Expert Manual (adjusted p = 0.049).
Supplementary Figure S8 shows the representative doublet gates.}
...
{\color{revisionred}\sout{4.}}{\color{revisionblue}S8; Supplementary
Table S5B.}
...
of those}}{\color{revisionblue}\texttt{passed\_lod} channel, treating}
values {\color{revisionred}\sout{is}}{\color{revisionblue}below 0.5 as
removed. The rule did} not
{\color{revisionred}\sout{yet}}{\color{revisionblue}activate in any of
the eight PBMC samples. In the three tumour samples, it removed
3,472--9,206 events, corresponding to 2.39--3.05\% of each input. The}
complete {\color{revisionblue}sample-level counts} and
