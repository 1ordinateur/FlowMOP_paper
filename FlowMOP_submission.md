...
To date, cytometry preprocessing algorithms have been evaluated either by comparison to human-defined gold standards (as in FlowCut and PeacoQC) or through mathematical metrics (consider cyCombine’s approach to batch correction [11]). Conversely, this paper seeks to compare performance not only through traditional human-expert defined standards, but also the development of bespoke data generated explicitly for pre-processing validation. This will allow for the true ‘cleaning’ ability of both algorithms and humans to be objectively compared to a ground truth. Accordingly, the proposed methodology facilitates the detection of algorithms that potentially outperform manual approaches—a capability current methods do not adequately provide.
